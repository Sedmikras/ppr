...
namespace percentile_finder {
	...
	uint32_t NumberMasker::return_index_from_double_second_stage(double number) {
		if (fpclassify(number) == -1 || fpclassify(number) == 0) {
	...
	uint32_t NumberMasker::return_index_from_double_last_stage(double number) {
		if (fpclassify(number) == -1 || fpclassify(number) == 0) {
			if (number >= low && number < high) {
				int64_t num = *(int64_t*)&number;
				if ((num >> 63) == 1) {
